...
async function seed() {
  ...
  // Clean up any old orders and proofs for this buyer to prevent duplicates in E2E tests
  console.log("Cleaning up old test buyer orders and payment proofs...");
  const { data: oldOrders } = await supabase.from("orders").select("id").eq("buyer_id", buyer.id);
  const oldOrderIds = oldOrders?.map(o => o.id) || [];
  if (oldOrderIds.length > 0) {
    await supabase.from("payment_proofs").delete().in("order_id", oldOrderIds);
