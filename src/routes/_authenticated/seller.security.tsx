    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Protect your seller account with 2FA and active session control.
        </p>
      </div>

      <PanelCard
        title="Password & two-factor"
        action={<Shield size={14} className="text-muted-foreground" />}
      >
        <div className="space-y-4 text-sm">
          <Row label="Password" desc="Last changed 28 days ago" cta="Change" />
          <Row
            label="Two-factor authentication"
            desc="Authenticator app · enabled"
            cta="Manage"
            enabled
          />
      ...
      <PanelCard
        ...
        action={<Lock size={14} className="text-muted-foreground" />}
      >
        <ul className="divide-y divide-border/50">
          {sessions.map((s) => (
            <li key={s.device} className="py-3 flex items-center gap-4">
              <div className="size-10 rounded-lg bg-background/60 grid place-items-center text-muted-foreground">
                <s.icon size={16} />
              </div>
  ...
  return (
    <div className="flex items-center justify-between gap-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <button className="h-9 px-3 rounded-lg border border-border text-xs hover:bg-surface">
          {cta}
        </button>
      </div>
    </div>
