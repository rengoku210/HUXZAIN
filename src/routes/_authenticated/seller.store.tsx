    ...
    try {
      ...
      if (supabase) {
        ...
        if (data) {
          ...
          setSelectedTheme(themeId || "midnight");
          setThemeEnabled(status !== "disabled");
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .upsert({
          ...
          theme_color: `${selectedTheme}|${themeEnabled ? "enabled" : "disabled"}`,
                  ...
                  {themes.map((t) => {
                    ...
                    return (
                      <button
                        ...
                        className={`rounded-xl border p-2.5 text-left transition-all ${active ? "border-gold ring-1 ring-gold/20 bg-gold/5" : "border-border/60 hover:border-gold/30"} ${locked ? "opacity-40 cursor-not-allowed" : ""}`}
                      >
